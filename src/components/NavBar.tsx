  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-8 py-4 mt-6 mx-auto max-w-7xl w-[95%] bg-white/80 backdrop-blur-md shadow-[0_20px_40px_rgba(26,28,28,0.06)] rounded-full">
      ...
      <div className="hidden md:flex gap-8 items-center">
        ...
        <Link to="/works" className={navLinkClass('/works')}>Works</Link>
        <Link to="/services" className={navLinkClass('/services')}>Services</Link>
        <Link to="/testimonials" className={navLinkClass('/testimonials')}>Testimonial</Link>
        <Link to="/contact" className={navLinkClass('/contact')}>Contact</Link>
      </div>
      <Link to="/contact" className="group flex items-center gap-2 bg-zinc-900 text-white px-7 py-3.5 rounded-full font-barlow font-bold text-[13px] uppercase tracking-[0.1em] hover:bg-zinc-800 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 active:scale-95">
        Book A Free Meeting
